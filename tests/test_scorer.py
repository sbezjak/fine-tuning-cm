import pytest

from ft_cm.scorer import extract_label, is_correct


@pytest.mark.mocked
def test_clean_labels():
    assert extract_label("safe").label == "safe"
    assert extract_label("unsafe").label == "unsafe"


@pytest.mark.mocked
def test_unsafe_is_not_mislabeled_safe():
    """The core trap: 'safe' is a substring of 'unsafe'. Word-boundary matching
    must read 'unsafe' as unsafe, not accidentally as safe."""
    ext = extract_label("unsafe")
    assert ext.label == "unsafe"
    assert not ext.ambiguous
    assert is_correct("unsafe", "unsafe")
    assert not is_correct("unsafe", "safe")


@pytest.mark.mocked
def test_label_inside_a_sentence():
    assert extract_label("This message is safe to publish.").label == "safe"
    assert extract_label("I would mark this as unsafe.").label == "unsafe"


@pytest.mark.mocked
def test_hedge_is_ambiguous_and_wrong():
    ext = extract_label("It could be safe or unsafe depending on context.")
    assert ext.ambiguous
    assert not is_correct("safe or unsafe", "safe")


@pytest.mark.mocked
def test_no_label_is_none_and_wrong():
    ext = extract_label("I cannot help with that.")
    assert ext.label is None
    assert not is_correct("I cannot help with that.", "safe")


@pytest.mark.mocked
def test_case_insensitive():
    assert extract_label("UNSAFE").label == "unsafe"
    assert is_correct("Safe", "safe")
